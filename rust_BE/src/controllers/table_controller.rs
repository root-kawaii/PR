...
pub async fn get_tables_by_event(
    ...
    match table_persistence::get_public_tables_by_event_id(&state.read_db_pool, event_uuid).await {
        Ok(tables) => {
            let table_responses: Vec<TableResponse> =
                tables.into_iter().map(|t| t.into()).collect();
...
pub async fn get_available_tables_by_event(
    ...
    match table_persistence::get_public_available_tables_by_event_id(
        &state.read_db_pool,
        event_uuid,
    )
    .await
